...
export const Banano = ({ fill, width }) => {
  return (
    <svg
      width={width || "18px"}
      viewBox="0 0 256 135"
...
const formatBan = (value, fill, direction) => {
  ...
  return (
    ...
    >
      {!reverse && <Banano fill={fill} />}
      <div
        style={{
          marginLeft: !reverse && "0.25rem",
          marginRight: reverse && "0.25rem",
        }}
      >
        {formatter.format(parseFloat(value)).replace("$", "")}
      </div>
      {reverse && <Banano fill={fill} />}
    </div>
  );
